  ...
  * @brief   Ejercicio Practica 4 Ejercicio 1
...
static delay_t delayANTIBOUNCE;
...
int main(void)
{
  /* STM32F4xx HAL library initialization:
	...
	delay_t delayANTIBOUNCE;

	delayInit( &delayANTIBOUNCE , DELAY_40_ms);
...
void debounceFSM_update(void){
	...
	case BUTTON_UP:
		if(BSP_PB_GetState(BUTTON_USER)){
			delayRead (&delayANTIBOUNCE);
			estadoMEF = BUTTON_FALLING;
		}
		break;

	case BUTTON_FALLING:
		if(delayRead(&delayANTIBOUNCE)){
			if(BSP_PB_GetState(BUTTON_USER)){
...
/**
  * @brief  Funcion que solamente realiza el encendido del LED como respuesta del boton presionado
  * @param  None
  * @retval None
  */
void buttonPressed(void){
...
/**
  * @brief  Funcion que solamente realiza el apagado del LED como respuesta del boton presionado
  * @param  None
  * @retval None
  */
