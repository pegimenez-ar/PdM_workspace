...
int main(void)
{
  /* STM32F4xx HAL library initialization:
	...
	Led_TypeDef led[]={LED1,LED2,LED3};		//Se define un array de leds para el manejo

	for( index=0 ; index < NUMERO_LEDS ; index++ ){
		BSP_LED_Init(led[index]);
	}
  ...
  /* Infinite loop */
	while (1)
	{
		/* Se comienza a recorrer el array de delays para observar si hay compleato el tiempo
		 * y se hace el cambio de estado del led (ON->OFF o OFF->ON)
		 */
		for( index=0 ; index < NUMERO_LEDS ; index++){
			if(delayRead(delay_led + index)){
				BSP_LED_Toggle(led[index]);
			}
		}
